...
def build_cdn():
    ...
    for dirpath, _, filenames in os.walk(SOURCE_DIR):
        ...
        for file in filenames:
            if file.endswith(('.pdb', '.formaldev', '.lib', '.exp', '.lnk', '.log', '.xml', '.lastbuildstate')):
                continue
            ...
            # Skip files > 95MB (GitHub limit = 100MB)
            file_size = os.path.getsize(full_path)
            if file_size > 95 * 1024 * 1024:
                print(f"  [SKIP] {rel_path} ({file_size / 1024 / 1024:.1f}MB) - quá lớn cho GitHub")
                continue
            ...
            # SHA256 của file GỐC (dùng làm object URL cho downloader)
            # Lưu file KHÔNG NÉN: khi CompressedSize == Size, parser set algorithm=None
            # và downloader sẽ fetch file raw không cần decompress

            # Tạo node XML với đúng format mà C++ parser expect
            # CompressedSize == Size → file không nén → algorithm=None → no suffix
            ET.SubElement(root_xml, "ContentFile",
                Name=rel_path,
                Size=str(size),
                CompressedSize=str(size),
                SHA1Hash=sha1,
                SHA256Hash=sha256)

            # Lưu file GỐC vào cấu trúc CDN dùng SHA256 hash làm tên
            h1 = sha256[0:2]
            h2 = sha256[2:4]
            out_dir = os.path.join(CDN_OUT, h1, h2)
            os.makedirs(out_dir, exist_ok=True)

            out_file = os.path.join(out_dir, sha256)
